...
describe("VectorSpace test suite", () => {
    it("Can be constructed from a number", () => {
        const vectorSpace = new VectorSpace(3);
    ...
    it("Can be constructed from an array", () => {
        const vectorSpace = new VectorSpace([new Vector([1, 2, 3]), new Vector([4, 5, 6])]);
        expect(vectorSpace).to.not.be.null;
    ...
    it("Can be cast to a ColumnsMatrix", () => {
        const vectorSpace = new VectorSpace([new Vector([1, 2, 3]), new Vector([4, 5, 6])]);
        const M = vectorSpace.toColumnsMatrix();
    ...
    it("Can be cast to a RowsMatrix", () => {
        const vectorSpace = new VectorSpace([new Vector([1, 2, 3]), new Vector([4, 5, 6])]);
        const M = vectorSpace.toRowsMatrix();
        expect(M).to.not.be.null;
        expect(M.m).equal(2);
        expect(M.n).equal(3);
        expect(M.elements[0][0].equals(1)).to.be.true;
    ...
    it("Can find a basis", () => {
        const vectorSpace = new VectorSpace([new Vector([1, 1, -2, 0, -1]),
            new Vector([1, 2, 0, -4, 1]), new Vector([0, 1, 3, -3, 2]), new Vector([2, 3, 0, -2, 0])]);
        const basis = vectorSpace.findBasis();
